...
for nn = 1:size(data,3)
    for ss = 1:nb_subtype
        data(sub(nn).part==ss,:,nn) = data(sub(nn).part==ss,:,nn) - repmat(sub(nn).map(ss,:),[sum(sub(nn).part==ss) 1]);
    end
end
...
for nn = [2 5 7]
    ...
    model.x = [ones(size(sub(nn).weights,1),1) tab(:,4) tab(:,7) tab(:,1) tab(:,2)]; % AD-MCI
