...
nb_subj <- 100 # number of total subjects
step_seq <- 10 # step sequence to encrement the nuber of subjects
percentil <- c(.25, .50, .75) # the sliding ratio MZ/DZ

for (nSub in seq(step_seq,nb_subj,10)){
...
#     colnames(alltwin)=c("MZ_twin1","MZ_twin2","DZ_twin1","DZ_twin2")
    ...
    if (nMz == seq_Mz[1]) {
      TabResult <- matrix(, nrow = 1, ncol = 6) # empty matrix to hold results for each fir times point
