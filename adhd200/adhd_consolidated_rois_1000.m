...
file_tsv = 'adhd200_preprocessed_phenotypics.tsv';
tab = niak_read_csv_cell(file_tsv);
...
for ss = 1:length(list_subject)
    ...
    if nb_files == 0 
        warning(sprintf('Could not find files for subject %s',subject));
    end
end
local_files = dir('.');
local_files = {local_files.name};
local_files(~ismember(local_files,all_files));
