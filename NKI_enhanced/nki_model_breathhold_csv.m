...
% generate group and individual model for motor task NKI_enhanced database.
%
% SYNTAX:
% [] = NKI_MODEL_MOTOR_CSV(OPT)
%
% _________________________________________________________________________
% root_path (string, default [pwd]) the full path to the root folder that contain the 
%   NKI Preprocessed data. 
%
% OPT
...
if isempty(root_path)
  ...
  elseif strfind(server,'ip05') % this is mammouth
      ...
      switch server
          ...
          root_path = '/media/scratch2/NKI_enhanced/';
          fprintf ('server: %s\n',server)
          my_user_name = getenv('USER');
...
mkdir([root_path 'fmri_preprocess_ALL_task/'], 'EVs/');
data.dir_output         = [root_path 'fmri_preprocess_ALL_task/EVs/'];
data.name_csv_group     = 'nki_model_group';
data.name_csv_intrarunLR  = 'nki_model_intrarunLR';
data.name_csv_intrarunRL  = 'nki_model_intrarunRL';
