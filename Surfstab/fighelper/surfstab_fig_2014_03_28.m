...
clear;
search_path = '/home/surchs/Project/surfstab/local/simus/three/manual/';
data_pattern = 'simu_var_\w*.mat';
out_path = '/home/surchs/Project/surfstab/local/simus/three/manual/figures';
...
for item = search_dir
    if item.isdir
        ...
        elseif strcmp(show_me, 'corr_core')
            ...
            for sc_id = 1:num_scale
                ...
                switch show_me
                    ...
                    case 'stab_core'
                        ...
                        % Get the partition for masking
                        part_ref_path = [cur_path filesep 'msteps_part.mat'];

                        if exist(part_ref_path, 'file')
                            part_cur_f = load(part_ref_path);
                        else
                            part_ref_path = [cur_path filesep 'part.mat'];
                            if exist(part_ref_path, 'file')
                                part_cur_f = load(part_ref_path);
                            else
                                error('Could not find a reference partition\n');
                            end
                        end
                        part = part_cur_f.part(:, sc_id);
                        % Work Pierre's magic
                        [~, part_ind] = min(abs(ref.s - scale));
                        ref_part = ref.part(:, part_ind);
                        % Map the partition to the reference
                        tmp_p = niak_match_part(part, ref_part);
                        part  = tmp_p.part2_to_1;
                        core_num = unique(part(ref_part==show_zone));
                        % Get the correct core
                        core = stab(core_num, :);
                        show = reshape(core, [ref.n ref.n]);
...
for name_id = 1:length(use_fields)
    ...
    for par_id = 1:num_files
        ...
        if ~isempty(show)
            % Set the color options - depending on the condition
            if ~strcmp(show_me, 'partition')
