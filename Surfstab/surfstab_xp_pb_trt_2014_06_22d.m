...
path_out = [path_data 'xp_pb_trt_2014_06_22d' filesep];
...
for k = 1:6
    subplot(2,3,k), voltmp = niak_part2vol (mean(y(:,k),2),mask); niak_montage (voltmp(:,:,33),opt_v);
end
...
for ss = 1:length(list_subject)
    ...
    for rr = 1:3
        cd([path_out subject '_sess' num2str(rr)])
        [hdr,vol] = niak_read_vol ('dual_regression.mnc.gz'); 
        vol = vol(:,:,:,8);
        if (ss==1)&&(rr==1)
            y = zeros(sum(mask(:)),length(list_subject));
        end
