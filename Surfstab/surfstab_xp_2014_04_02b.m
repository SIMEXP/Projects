...
opt_s.variance = 2; % The variance of the signal to simulate the cluster structure. The i.i.d. has a variance of 1, so a choice of 1 here corresponds to a SNR of 1. 
...
% Sampling
opt.sampling.type = 'scenario';
opt.sampling.opt = opt_s;
% Flags
opt.flag_cons = true;
opt.flag_cores = true;
opt.flag_rand = false;
...
opt_p.thresh = opt_s.nb_clusters;
part = niak_threshold_hierarchy(hier, opt_p);
out_p.part = part;
out_p.order = order;
part_name = 'prepart.mat';
part_path = [path_simu part_name];
save(part_path, '-struct', 'out_p');
