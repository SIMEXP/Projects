...
for fwhm_id = 1:length(range_fwhm)
    ...
    for var_id = 1:length(range_variance)
        ...
        opt.flag_test = true;
        opt.psom.qsub_options = '-q sw -l nodes=1:ppn=2,walltime=00:50:00';
        ...
        % Call the pipeline
        sub_pipe = niak_pipeline_stability_surf(in,opt);
        % Change names of jobs
        jobs = fieldnames(sub_pipe);
        for job_id = 1:length(jobs)
            job_name = jobs{job_id};
            new_name = sprintf('pipe_%d_%s', count, job_name);
            sub_pipe.(new_name) = sub_pipe.(job_name);
            sub_pipe = rmfield(sub_pipe, job_name);
        end  
        pipe = psom_merge_pipeline(pipe, sub_pipe);
        count = count + 1;
    end
end
