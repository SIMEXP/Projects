...
%% Just simple t-stats on conn / age relatioship
exp = [ones(length(age),1) age];
x = exp;
x(:,2:end) = niak_normalize_tseries(exp(:,2:end));
...
%% Anatomy-based prediction of age
anat = [ones(size(bvol)) bvol csf gm];
...
for ss = 1:length(list_subject)
    ...
    beta = niak_lse(y,(x-repmat(mx,[size(x,1) 1]))./repmat(sx,[size(x,1) 1]));
    
    % Mage the prediction
    age_hat(ss) = ((anat(ss,:)-mx)./sx)*beta;
end

%% Now try to predict age with a combination of functional connectivity and anatomy
list_c = 2.^(-7:2:10);
list_g = 2.^(-10:2:5);
K = 10;
n = 8;
