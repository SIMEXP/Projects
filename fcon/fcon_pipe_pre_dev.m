function [pipeline,files_in,files_out,opt] = fcon_pipe_pre_dev(files_in,files_out,opt)
...
%     Structure containing:
%       path_mnc:
%         Path where to save the converted nii.gz files. By default: $path_datbases/converted/$database.
%       path_output:
%         Path where to save output files. Will be created if non-existant. By default: $path_database/output/.
%       path_demog:
%         Path to the demographics file of the database. By default: $path_database/$database_demographics.txt.
%       path_release_table:
%         Path to the release table for all databases. By default: $path_databases/fcon_1000_ReleaseTable.csv.
%       path_preprocess:
%         Path where to save the preprocessed files. Will be created if non-existant. By default: $path_database/preprocess/$database.
%       path_logs:
%         Path where pipeline log files are kept. See psom_run_pipeline for more information. Will be created if non-existant. By default: $path_database/logs/.
%       flag_test:
...
%       restart:
%         List of jobs to restart. By default: empty.
%
% OUT:
%   pipeline:
...
gb_list_fields = {'path_logs','path_pipeline'};
gb_list_defaults = {'/database/fcon_1000/logs/all/fmri/';'/database/fcon_1000/pipelines/'};
niak_set_defaults;
