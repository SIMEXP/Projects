...
for num_net = 1:12
    ...
    for cc = 1:max(part)
        ...
        %weights(:,cc) = sum((tseries_ga'-repmat(avg_clust(cc,:)',[1 size(tseries,1)])).^2,1);
        %[beta,e,std_e,ttest] = niak_lse(tseries_ga',[ones(size(avg_clust(cc,:)')) niak_normalize_tseries(avg_clust(cc,:)') niak_normalize_tseries(avg_clust(cc,:)').^2]);
        %weights(:,cc) = ttest;
    end
