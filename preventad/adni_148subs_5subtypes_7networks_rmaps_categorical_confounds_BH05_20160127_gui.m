...
f = dir(path_data_1);
[~, path_name, ~] = niak_fileparts(path_data_1);
in_strings = {f.name};
in_strings = in_strings(3:end);

for nnet = 1:num_scale
    for id = 1:length(in_strings)
        sub_template = in_strings{id};
        file_sub_vol = [path_data_1,sub_template];
        [hdr,sub_vol] = niak_read_vol(file_sub_vol);
        stack(:,:,:,id) = sub_vol(:,:,:,nnet);
    end
    hdr.file_name = [path_results,'stack_net',num2str(nnet),'.mnc.gz'];
    niak_write_vol(hdr,stack);
    mean_stack(:,:,:,nnet) = mean(stack,4);
    std_stack(:,:,:,nnet) = std(stack,0,4);
end

hdr.file_name = [path_results,'stack_mean','.mnc.gz'];
niak_write_vol(hdr,mean_stack);
hdr.file_name = [path_results,'stack_std','.mnc.gz'];
...
for n_net = 1:length(num_net)
    ...
    % Load data
    file_stack1 = [path_results,'stack_net',num2str(num_net(n_net)),'.mnc.gz'];
    [hdr,stab1] = niak_read_vol(file_stack1);
    
    [hdr,mask] = niak_read_vol([path_mask 'mask.mnc.gz']);
    ...
    if save_figs_clust ==1
        ...
        hdr.file_name = [path_res_net name '_mean_clusters.mnc.gz'];
        ...
        hdr.file_name = [path_res_net name '_mean_cluster_demeaned.mnc.gz'];
        niak_write_vol(hdr,vol_avg);
        
        hdr.file_name = [path_res_net name '_grand_mean_clusters.mnc.gz'];
