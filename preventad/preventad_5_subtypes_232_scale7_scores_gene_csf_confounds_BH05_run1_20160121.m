...
num_var = [5 6 7 8 9 10 11 12]; % num variable in model.csv
name_var = {'apoe4','bcheK','bdnf','hmgr','Tau','Beta','pTau','apoe_csf'}; % name variable in model.csv
type_plot = [1 1 1 1 2 2 2 2]; % 1=boxplot, 2=regression line
num_fd1 = 20; % num FD variable in model.csv, used as confounding variable
num_age = 1;
num_sex = 2;
...
template_file = 'rest1_stability_maps.nii.gz';
path_data_1 = '/Users/pyeror/Work/transfert/PreventAD/results/scores/preventad/stability_maps_run1/';
path_results = '/Users/pyeror/Work/transfert/PreventAD/results/subtype/preventad_232_s7_5subtypes_scores_gene_csf_BH05_confounds_run1/';
...
%% Create main output directory manually and copy stack and mask maps in there
...
for n_net = 1:length(num_net)
    ...
    % Load data
    file_stack1 = [path_results,'stack_net',num2str(num_net(n_net)),'.nii.gz'];
    [hdr,stab1] = niak_read_vol(file_stack1);
    
    [hdr,mask] = niak_read_vol([path_results 'mask.nii.gz']);
    ...
    save([path_results 'order.mat'],'order');
