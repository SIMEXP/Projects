...
% association
num_var = [8 9 10]; % num variable in model.csv
name_var = {'ctrlvsmci','ctrlvsad','mcivsad'}; % name variable in model.csv
num_fd = 5; 
num_age = 3;
num_sex = 2;
qfdr = 0.05;
...
% paths
model = '/Users/pyeror/Work/transfert/PreventAD/models/admci_model_subtype_70vols_20160220.csv';
path_data = '/Users/pyeror/Work/transfert/PreventAD/results/scores/admci_7/rmap_part_stack/';
path_mask = '/Users/pyeror/Work/transfert/PreventAD/results/scores/';
path_results = '/Users/pyeror/Work/transfert/PreventAD/results/subtype/admci_20160221_subtype2/';
...
for n_net = 1:length(num_net)
    ...
    for n_var = 1:length(num_var)
        ...
        age = tab2(:,num_age);
        sex = tab2(:,num_sex);
        model_covar.x = [ones(sum(mask_covar),1) niak_normalize_tseries([covar(mask_covar) fd(mask_covar) sex(mask_covar) age(mask_covar)],'mean')];
