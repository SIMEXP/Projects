%% regress sex age fd sites before subtypes, based on all ADNI2MTL data (ie select and sample are same variable)
...
% paths
model = '/Users/pyeror/Work/transfert/PreventAD/models/admci_model_20160401.csv';
path_data = '/Users/pyeror/Work/transfert/PreventAD/results/scores/admci_7_20160401/rmap_part_stack/';
path_regions = '/Users/pyeror/Work/transfert/PreventAD/templates_nii/template_cambridge_basc_multiscale_sym_scale444.nii.gz';
path_mask = '/Users/pyeror/Work/transfert/PreventAD/results/scores/';
path_results = '/Users/pyeror/Work/transfert/PreventAD/results/subtype/admci_20160401_subtype1/';
...
num_multisite = 5;
qfdr = 0.05;
type_plot = 1; % 1=boxplot, 2=regression line
...
for n_net = 1:length(num_net)
    ...
    [hdr,mask_vox] = niak_read_vol([path_mask 'mask.nii.gz']);
    
    if region == 1
        [hdr_reg,vol_reg] = niak_read_vol(path_regions);
        [raw_data,~,~] = niak_build_tseries(stack,vol_reg);
        mask = vol_reg;
    else
        raw_data = niak_vol2tseries(stack,mask_vox);
        mask = mask_vox;
        
    end
    
    % Extract betas and regress confounds 
    x = [ones(size(raw_data,1),1) tab(:,num_sex) tab(:,num_age) tab(:,num_fd) tab(:,num_mnimci) tab(:,num_criugmad) tab(:,num_criugmmci) tab(:,num_adni5)]; 
    y = raw_data;
    betas = (x'*x)\x'*y;
    data = y-x*betas;
    ...
    if fig_clust ==1
        ...
        for hh = 1:length(name_grp)
            ...
            namefig = [path_res_net 'pie_' name_grp{hh} '.pdf'];
            print(namefig,'-dpdf','-r300')
            close all
        end
    ...
    for n_var = 1:length(num_var)
        ...
        for n_site = 1:max(tab(:,num_multisite))
            f2tmp(n_site,:)=res_covar.single_site.results(n_site).f2;
        end
        nb = tab(:,num_multisite);
        f2(n_var,:,n_net) = (f2tmp(1,:)*(numel(find(nb==1)))+f2tmp(2,:)*(numel(find(nb==2)))+f2tmp(3,:)*(numel(find(nb==3)))+f2tmp(4,:)*(numel(find(nb==4)))+f2tmp(5,:)*(numel(find(nb==5))))/size(tab,1);
...
path_res_f2 = [path_results 'f2.csv'];
...
opt.precision = 3;
niak_write_csv(path_res_f2,f2_csv,opt);
...
opt.labels_y = {'Vchi2'};
opt.precision = 3;
path_V = [path_results 'chi2_V.csv'];
niak_write_csv(path_V,Vchi,opt)
