...
list_run = {'LR' , 'RL'};
path_mnc = path_raw ;
path_nii = [ root_path  'HCP_raw_mnc/' ];
...
for num_s = 1:nb_subject
    ...
    fprintf('Subject %s\n',subject);
    path_sub = [path_mnc subject filesep];
    path_read.func = [path_sub 'unprocessed/3T'];
    path_read.anat = [path_sub 'unprocessed/3T/T1w_MPR1'];
    path_write.anat = [path_nii subject filesep 'MPR_1' filesep];
    for num_r = 1:length(list_ run)
          run = list_run{num_r};
          for num_t = 1:length(list_task)
              task = list_task{num_t};
              fprintf('    Task  %s_%s\n', task , run);
              path_write.func = [path_nii subject filesep task filesep];
              path_tmp = [path_nii subject filesep task filesep 'tmp' filesep];
              name_job = subject ;
              ...
              pipeline.(name_job).files_out.func = [path_write 'func_' subject '_' task '.nii.gz'];
              pipeline.(name_job).files_out.anat = [path_write 'anat_' subject '_' task '.nii.gz'];
