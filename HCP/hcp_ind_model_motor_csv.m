...
switch opt.run
     case 'lr'
        ...
        if  ismember(opt.trial,{'run1'})
            model_intrarun_cond  = {opt.trial,'baseline'};
            model_intrarun_values(1,1) = str2num(niak_read_csv_cell([path_folder  'rh.txt'],opt_csv_read){1,1})- 3 - opt.trial_delay;
     ...
     case 'rl'
        opt_csv_read.separator= sprintf('\t');
        model_intrarun_names = {'times','duration'};
        if  ismember(opt.trial,{'run2'})
            model_intrarun_cond  = {opt.trial,'baseline'};
            model_intrarun_values(1,1) = str2num(niak_read_csv_cell([path_folder  'lh.txt'],opt_csv_read){1,1})- 3 - opt.trial_delay;
            model_intrarun_values(1,2) = opt.trial_duration;
