%%% ADNET GLM CONNECTOME SCRIPT - MCI VS CONTROLS ONLY - basc 40 msteps scales -
%%% NOTE: adni2 achieva scanners only
...
%% Options 
%%%%%%%%%%%%
opt.folder_out = [path_data 'adnet/results/glm30b_achieva_20151116']; % Where to store the results
...
opt.test.ctrlvsmci.group.select(2).values = 5;
...
opt.test.avg_ctrl.group.select(2).values = 5;
...
opt.test.avg_mci.group.select(2).values = 5;
