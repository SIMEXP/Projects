%%% MCINET GLM CONNECTOME SCRIPT for MSTEPS resolutions in the ADNI2 SAMPLE
%%% This GLM takes into account different scanner models within the ADNI2 cohort
...
%% Options 
%%%%%%%%%%%%
opt.folder_out = [path_data 'adnet/results/glm30b_adni2_scanners_20151117']; % Where to store the results
...
opt.test.ctrlvsmci.group.select(2).values = [5 6 7 9];
...
opt.test.avg_ctrl.group.select(2).values = [5 6 7 9];
...
opt.test.avg_mci.group.select(2).values = [5 6 7 9];
