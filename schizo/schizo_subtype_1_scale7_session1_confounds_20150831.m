...
num_var = [4 8 9 10 11 ]; % num variable in model.csv
...
path_results = '/Users/pyeror/Work/transfert/Schizo/results/subtype/scores_s7/session1_BH05_confounds_20150831/';
...
for n_net = 1:length(num_net)
    ...
    for n_var = 1:length(num_var)
        ...
        fd1 = tab2(:,num_fd1);
        study = tab2(:,num_study);
        age = tab2(:,num_age);
        sex = tab2(:,num_sex);
        model_covar.x = [ones(sum(mask_covar),1) niak_normalize_tseries([covar(mask_covar) fd1(mask_covar) study(mask_covar) age(mask_covar) sex(mask_covar) ],'mean')];
        model_covar.y = weights1(mask_covar,:);
        covar1 = model_covar.y;
        model_covar.c = [0;1;0;0;0;0];
        opt_glm.test = 'ttest';
