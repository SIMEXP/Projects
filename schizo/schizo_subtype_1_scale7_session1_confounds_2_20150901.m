...
num_var = [6 10 11 12 13]; % num variable in model.csv
name_var = {'schizo','pansspos1','panssneg1','panssgen1','pansstot1'}; % name variable in model.csv
type_plot = [1 2 2 2 2]; % 1=boxplot, 2=regression line
num_study1 = 2;
num_study2 = 3;
num_fd1 = 9; % num FD variable in model.csv, used as confounding variable
num_age = 7;
num_sex = 8;
nb_clus = 5; % nb clusters in clustering
name_clus = {'subt1','subt2','subt3','subt4','subt5'};
model = '/Users/pyeror/Work/transfert/Schizo/models/schizo_model_group.csv';
path_data_1 = '/Users/pyeror/Work/transfert/Schizo/results/scores/scores_s7/session1/stability_maps_nii/';
template1 = 'fmri_%s_session1_run1_stability_maps.nii.gz';
path_results = '/Users/pyeror/Work/transfert/Schizo/results/subtype/scores_s7/session1_BH05_confounds_2_20150901/';
save_figs_clust = 0; % 1 = save, 0 do not save
save_figs_plots = 0; % 1 = save, 0 do not save
...
for n_net = 1:length(num_net)
    ...
    for n_var = 1:length(num_var)
        ...
        fd1 = tab2(:,num_fd1);
        study1 = tab2(:,num_study1);
        study2 = tab2(:,num_study2);
        age = tab2(:,num_age);
        sex = tab2(:,num_sex);
        model_covar.x = [ones(sum(mask_covar),1) niak_normalize_tseries([covar(mask_covar) fd1(mask_covar) sex(mask_covar) age(mask_covar) study1(mask_covar) study2(mask_covar)],'mean')];
        model_covar.y = weights1(mask_covar,:);
        covar1 = model_covar.y;
        model_covar.c = [0;1;0;0;0;0;0];
        opt_glm.test = 'ttest';
