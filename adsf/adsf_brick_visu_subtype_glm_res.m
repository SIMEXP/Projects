function [files_in,files_out,opt] = adsf_brick_visu_subtype_glm_res(files_in, files_out, opt)
% Generating graphs to visualize results of subtype association tests
% (residuals)
%
% SYNTAX: [FILE_IN,FILE_OUT,OPT] =
% ADSF_BRICK_VISU_SUBTYPE_GLM_RES(FILE_IN,FILE_OUT,OPT)
...
if ~isempty(opt.folder_out)
    ...
    files_out = psom_struct_defaults(files_out,...
                { 'figures'                                                       },...
                { make_paths(path_out, 'fig_association_res_net_%d.pdf', 1:opt.scale) });
else
    files_out = psom_struct_defaults(files_out,...
                { 'figures'         },...
                { 'gb_niak_omitted' });
    ...
    for net_id = 1:opt.scale
        ...
        for sbt_id = 1:n_sbt
            ...
            if coi_cat
                ...
                plot_model.x = sbt_weights;
                plot_model.y = resid;
                
                % Make the scatterplot
                hold on;
                %plot(sbt_weights, coi, '.k');
                plot(plot_model.x, plot_model.y, '.k');
                %plot(x_fit, y_fit, 'r');
                lsline
                h = lsline;
                set(h(1),'color','r')
