...
for nn = 1:4 % for every network
    
    net = nb_net(nn)
    % get the right ct regressor for the network
    opt.regress_conf{8} = strcat('sc11_mean_ct_net',num2str(net));
    ...
    % Save the new stack
    fname = strcat('/home/angela/Desktop/adsf/ct_subtypes/admci/basc_sc11_stacks/regress_agesexmeanct/ct_network_',num2str(net),'_stack.mat');
