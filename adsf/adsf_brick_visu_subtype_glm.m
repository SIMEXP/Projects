function [files_in,files_out,opt] = adsf_brick_visu_subtype_glm(files_in, files_out, opt)
% Generating graphs to visualize results of subtype association tests
%
% SYNTAX: [FILE_IN,FILE_OUT,OPT] =
% adsf_BRICK_visu_subtype_glm(FILE_IN,FILE_OUT,OPT)
    ...
    for net_id = 1:opt.scale
        ...
        for sbt_id = 1:n_sbt
            ...
            if coi_cat
                ...
                % Put the groups into cells
                for cc = 1:n_unique
                    sbt_cell{cc} = sbt_weights(coi == coi_unique(cc));
                end
                if is_octave
                    % Use the cells for boxplots
                    boxplot(sbt_cell);                  
                else
                    % The groups can be in a vector for the boxplot
                    boxplot(sbt_weights, coi, 'symbol','w');
                end
                hold on
                % Plot the data points over top of the box plot
                pplace = fliplr(1:n_unique);
                pplace = pplace-(max(pplace)-1);
                for cc = 1:n_unique
                    idx = 2-pplace(cc);
                    plot(idx-0.1+0.2*rand(size(sbt_cell{cc},1),1),sbt_cell{cc},'.','Marker','o','MarkerSize',0.5,'MarkerFaceColor','black','MarkerEdgeColor','black');
                end
