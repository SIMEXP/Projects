%% script to regress covariates (age, gender, mean global cortical thickness, mean network cortical thickness) 
% out of raw ct network stacks
...
opt.regress_conf{3} = 'mean_ct_whole_brain';
...
for net = 1:9 % for every network
    ...
    opt.regress_conf{4} = strcat('mean_ct_net',num2str(net));
    ...
    % Save the new stack
    fname = strcat('/Users/AngelaTam/Desktop/adsf/ct_stack_age_gender_mean_ctnet_wb/ct_network_',num2str(net),'_stack.mat');
