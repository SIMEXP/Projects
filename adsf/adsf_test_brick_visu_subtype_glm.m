function [files_in,files_out,opt] = adsf_test_brick_visu_subtype_glm(files_in, files_out, opt)
% Generating graphs to visualize results of subtype association tests
%
% SYNTAX: [FILE_IN,FILE_OUT,OPT] =
% ADSF_TEST_BRICK_VISU_SUBTYPE_GLM(FILE_IN,FILE_OUT,OPT)
    ...
    for net_id = 1:opt.scale
        ...
        for sbt_id = 1:n_sbt
            ...
            if coi_cat
                ...
                plot_model.x = [ones(size(sbt_weights)), sbt_weights];
                plot_model.y = coi;
                [res, ~] = niak_glm(plot_model, struct('flag_beta', true, 'flag_residuals', true));
                x_fit = linspace(min(sbt_weights),max(sbt_weights),10);
                y_fit = res.beta(1) + x_fit.*res.beta(2);
                % Make the scatterplot
                hold on;
                plot(sbt_weights, coi, '.k');
                plot(x_fit, y_fit, 'r');
                hold off;
                disp('done');
            end
