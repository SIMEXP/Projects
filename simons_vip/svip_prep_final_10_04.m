...
path_out = '/gs/project/gsf-624-aa/simons_vip/svip_prep_test_rest1_2_11_4/';

%path_raw = [root_path 'svip_database_rest2/'];
path_raw = [root_path 'svip_database_all2/'];
...
for num_s = 1:length(list_subject)
    ...
    rest2_flag_exist = dir([tmp_path_subj 'rest2/' '*.nii.gz']);
    if isempty (rest2_flag_exist) 
        warning('Subject %s is missing either the rest2',subject)
    else
        files_in.(id).fmri.session2.rest = [tmp_path_subj 'rest2/' dir([tmp_path_subj 'rest2/' '*.nii.gz'])(1).name];
    end
...
%% Pipeline options  %%
%% General
opt.folder_out   = '/gs/project/gsf-624-aa/simons_vip/svip_prep_test_rest1_2_10_27';      
