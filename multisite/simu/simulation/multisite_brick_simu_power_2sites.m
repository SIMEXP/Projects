function [files_in,files_out,opt] = multisite_brick_simu_power_2sites(files_in,files_out,opt)
% FILES_IN
...
for num_samp = 1:opt.nb_samps
    patho = zeros(size(X,1),1);
    Y_samp = Y;
    [samp_site,ctrl_samp] = sub_samp_2sites(multisite,X(:,2),alpha,n_subjects,opt.rnd_sampling);
    selection = [samp_site;ctrl_samp];
    for num_c = 1:nb_conn
...
function [samp_site,ctrl_samp] = sub_samp_2sites(multisiteidx,siteids,alpha,n_subjects,rnd_samp)
    ...
    patho_ratio = alpha(1);
    tmp_site = find(siteids == ids(randperm(length(ids))(1)));
    sub_samp_site = tmp_site(randperm(length(tmp_site)));
    if length(tmp_site)<20
	sub_samp_site = sub_samp_site(1:length(tmp_site));
        samp_site = [sub_samp_site(1:floor(length(sub_samp_site).*patho_ratio))];
        ctrl_samp = [sub_samp_site(~ismember(sub_samp_site,samp_site))];
    else
       	sub_samp_site = sub_samp_site(1:20);
       	samp_site = [sub_samp_site(1:floor(length(sub_samp_site).*patho_ratio))];
       	ctrl_samp = [sub_samp_site(~ismember(sub_samp_site,samp_site))];
    end
   
    % big site 80 subj
    patho_ratio = 1-alpha(1);
    tmp_site = find(siteids == 3);
    sub_samp_site = tmp_site(randperm(length(tmp_site)));
    sub_samp_site = sub_samp_site(1:80);
    samp_site = [samp_site;sub_samp_site(1:floor(length(sub_samp_site).*patho_ratio))];
    ctrl_samp = [ctrl_samp;sub_samp_site(~ismember(sub_samp_site,samp_site))];
end
