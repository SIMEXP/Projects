...
path_data = '/data/cisl/cdansereau/multisite/simulations_corr/2sites_realdata/'
...
for i_samp_size=1:size(sample_sizes,2)
   ...
   if sample_sizes(i_samp_size) == 100
        test_str = ['s' num2str(sample_sizes(i_samp_size)) '_30_70_2site'];
        ...
        for num_p = 1:length(list_p)
            for num_e = 1:length(list_effect_size)
                ...
                job.opt.force_balance = [0.30,0.70];
                pipeline = psom_add_job(pipeline,sprintf('simu_eff%i_p%i',100*list_effect_size(num_e),100*list_p(num_p)),'multisite_brick_simu_power_2sites',job.in,job.out,job.opt);
            end
        end
	...
	main_pipeline = psom_merge_pipeline(main_pipeline,pipeline,test_str);
         ...
         test_str = ['s' num2str(sample_sizes(i_samp_size)) '_70_30_2site'];
        ...
        for num_p = 1:length(list_p)
            for num_e = 1:length(list_effect_size)
                ...
                job.opt.seed_std = []; % std of the data after scrubbing 0.5
                job.opt.rnd_sampling = false;
                job.opt.force_balance = [0.70,0.30];
                pipeline = psom_add_job(pipeline,sprintf('simu_eff%i_p%i',100*list_effect_size(num_e),100*list_p(num_p)),'multisite_brick_simu_power_2sites',job.in,job.out,job.opt);
            end
        end
