...
#data(myTwinData)
exp = "exp1d"
manip = "4scrubb_permut_subj_sex1"
...
myTwinData$X <- NULL # remove extra colomn
...
for (cc in seq(cluster)) {
  for (vv in seq(volume)) {
    ...
    myTwinDataVars <- subset(myTwinData, sexe == 1 , c("id_scan","nofamill","sexe","zygotie",clust_vol_tmp)) #subset variable of interest
    myTwinDataVars  <- myTwinDataVars[allDup(myTwinDataVars$nofamill),] 
