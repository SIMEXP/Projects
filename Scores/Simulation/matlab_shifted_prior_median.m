...
fig_path = '/home/surchs/Code/Projects/Scores/Simulation/figures/shifted_right_median';
psom_mkdir(fig_path);
...
for net_id = 1:n_nets
    ...
    scores = squeeze(median(squeeze(scores_roc(:, 1, :, net_id, :)),2));
    seed = squeeze(median(squeeze(seed_roc(:, 1, :, net_id, :)),2));
    dureg = squeeze(median(squeeze(dureg_roc(:, 1, :, net_id, :)),2));
    ...
    for s_id = 1:n_shifts
        ...
        perc_shift = (shift*100)/16;
        % AUC
        tmp_scores_auc = median(scores_auc(:, net_id, s_id),1);
        tmp_seed_auc = median(seed_auc(:, net_id, s_id),1);
        tmp_dureg_auc = median(dureg_auc(:, net_id, s_id),1);
...
for net_id = 1:n_nets
    ...
    auc_all = [squeeze(median(scores_auc(:, net_id, :),1)), squeeze(median(seed_auc(:, net_id, :),1)), squeeze(median(dureg_auc(:, net_id, :),1))];
