...
nbSubj <- 1000 
raw.mzData <- mvrnorm (nbSubj, c(0,0), matrix(c(1,my_rMZ,my_rMZ,1),2,2))
raw.dzData <- mvrnorm (nbSubj, c(0,0), matrix(c(1,my_rDZ,my_rDZ,1),2,2))
# create  permutation betewn twin1 and twin2
permute <- 100
permMZ <- matrix(,nrow = dim(raw.mzData)[1], ncol = dim(raw.mzData)[2])
permDZ <- matrix(,nrow = dim(raw.dzData)[1], ncol = dim(raw.dzData)[2])
for (pp in seq(permute+1)) { #permutation test for twin1 and twin2 column
  if (pp == 1) {
    mzData = raw.mzData
    dzData = raw.dzData
  }else {
    for (nb in seq(nbSubj)) { # suffling twin1 and twin2
      set.seed(nb)
      permMZ[nb,] <- raw.mzData[nb,sample(ncol(raw.mzData))]
      set.seed(nbSubj+pp)
      permDZ[nb,] <- raw.dzData[nb,sample(ncol(raw.dzData))]
    }
    mzData = permMZ
    dzData = permDZ
  }
